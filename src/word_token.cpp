...
static ReservedWord rw3[] = {
    {(char *)"and", tcAND}, {(char *)"div", tcDIV},
    {(char *)"end", tcEND}, {(char *)"for", tcFOR},
    {(char *)"mod", tcMOD}, {(char *)"nil", tcNIL},
    {(char *)"not", tcNOT}, {(char *)"set", tcSET},
    {(char *)"var", tcVAR}, {NULL},
};

static ReservedWord rw4[] = {
    {(char *)"case", tcCASE}, {(char *)"else", tcELSE},
    {(char *)"file", tcFILE}, {(char *)"goto", tcGOTO},
    {(char *)"then", tcTHEN}, {(char *)"type", tcTYPE},
    {(char *)"with", tcWITH}, {NULL},
};
...
void WordToken::checkForReservedWord() {
  ...
  if ((len >= minReswordLen) && (len <= maxResWordLen)) {
    for (pReservedWord = rwTable[len]; pReservedWord->pString;
         ++pReservedWord) {
