...
const char *formStrings[] = {"*** Error ***", "Scalar", "Enumeration",
...
void Type::printTypeSpec(VerbosityCode vc) const {
  sprintf(list.text, "%s, size %d bytes. Type identifier: ", formStrings[form],
          size);
...
void Type::printEnumType(VerbosityCode vc) const {
  ...
  for (SymbolTableNode *pConstId = enumeration.pConstIds; pConstId;
...
void checkAssignmentCompatible(const Type *pTargetType, const Type *pValueType,
                               ErrorCode ec) {}
